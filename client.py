...
必要なもの（クライアントPC側）:
  pip install pynput sounddevice numpy pyperclip requests pystray Pillow
...
from client_gui import ClientTrayIcon, ClientHistoryWindow
...
class AirTypeClient:
    ...
    def __init__(self, tray: ClientTrayIcon, history: ClientHistoryWindow):
        print("=" * 50)
    ...
    def _stop_and_enqueue(self):
        ...
        try:
            ...
            threading.Timer(2.0, lambda: self._tray.set_state("idle")).start()
...
# ─────────────────────────────────────
def main():
    # tkinter root（非表示）: pystray / HistoryWindow のイベントループ
    root = tk.Tk()
    root.withdraw()  # メインウィンドウは表示しない

    history = ClientHistoryWindow(root)
    ...
    tray = ClientTrayIcon(root, on_history=history.show, on_quit=on_quit)
    client = AirTypeClient(tray=tray, history=history)
