...
_N_CTX  = "768"   # コンテキスト長（リファイナー用途は ~430トークン以内で十分）
_N_PRED = "256"   # 最大生成トークン数

_SYSTEM_PROMPT = (
    "あなたは優秀な文章校正アシスタントです。"
    "入力された音声認識テキストから『ね』『こう』『えっと』などのフィラーを削除し、"
    "必ず適切な句読点（、。）を補ってください。"
    "さらに、全体の文体を綺麗な『です・ます調』に統一して出力してください。"
    "挨拶や解説は不要です。整形後のテキストのみを出力してください。"
)

_FEW_SHOT_INPUT = (
    "プロジェクトの進直についてなんですが今週中には設計が完了する予定ね"
    "来週からは実装フェーズに入りたいと思っていますこうリソースが少し足りないかな"
)
_FEW_SHOT_OUTPUT = (
    "プロジェクトの進捗についてなんですが、今週中には設計を完了する予定です。"
    "来週からは実装フェーズに入りたいと思っていますが、リソースが少し足りないかもしれません。"
)
